...
class Parser:
  ...
  # <primary>  ::=  <integer> | <float> | <bool> | <identifier> | <string> | '(' <expr> ')'
  def primary(self):
  ...
  # <for_stmt> ::= 'for' <identifier> := <start> ',' <end> ( ',' <step> )? 'do' <stmts> 'end'
  def for_stmt(self):
      self.expect(TOK_FOR)
  ...
  def stmt(self):
    # Predicitive parsing, where the next token predicts what is the next statement
  ...
  def stmts(self):
    stmts = []
    # Loop all statments of the current block (meaning until we find an "end", or "else", or EOF)
